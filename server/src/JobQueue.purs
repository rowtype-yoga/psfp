...
newtype NewJob a = NewJob (JobId -> a -> Aff Unit)
...
newtype RunningJob a = RunningJob
  { id ∷ JobId
  , addedAt ∷ JSDate
  , startedAt ∷ JSDate
  , fiber ∷ Fiber Unit
...
newtype ResourcePool a = ResourcePool (Array a)
...
startJob q@(Queue { timeout } _ jobsRef) resource (PendingJob { id, job, addedAt }) = do
  ...
  let remove = removeFromQueue id q
  fiber <-
    launchAff <<< sequential
      $ parallel (job resource *> remove)
      *> parallel (delay timeout *> remove)
  let running = RunningJob { id, fiber, addedAt, startedAt, resource }
  jobsRef
...
enqueue (NewJob newJob) q@(Queue { maxSize, timeout } _ jobsRef) = do
  ...
  if length jobs >= maxSize then
    ...
    addedAt <- JSDate.now
    let new = Pending (PendingJob { id, addedAt, job: newJob id })
